...
27. 浏览页右上角在“设置”左侧新增来源模式按钮，默认网络，选择会持久化；未聚焦时只显示切换图标，遥控器聚焦时展开切换目标文字（网络模式显示“本地”，本地模式显示“网络”）。本地使用主共享外部存储目录树，隐藏根级连接管理与 Menu 连接管理入口；排序、焦点锚点、播放、收藏、历史、定位、同名 LRC、内嵌标签和同目录封面继续复用现有链路。本地首次进入会引导系统“所有文件访问权限”，未授权时显示“前往授权”。
...
30. 设置页左侧分类列表已改为标题固定、分类列表单独滚动；小屏触摸设备可纵向滑动触达“关于”等底部分类，遥控器焦点移动仍会自动滚动到可见区域。统一沉浸式全屏逻辑在主题层与 Activity `setContentView` 前共同声明 display cutout 策略，公共全屏 Modal 和设置页自定义全屏 Dialog 也会对独立窗口应用相同策略：Android P/Q 使用 `shortEdges`，Android R 及以上使用 `always` 并关闭 decor 的系统窗口适配，避免刘海屏横屏左侧继续被系统预留为只有背景、没有页面内容的安全区。
...
32. 全部交互界面补齐动效与微交互，且触屏与遥控器严格隔离，可复用逻辑收敛到 `ui/UiMotion.kt`。触屏点按按钮/文件行/设置项/modal 行时，由真实触摸（`MotionEvent`）驱动一层 overlay 色 `RippleDrawable`（`foreground`）产生“涟漪 + 加深/提亮”，并使用控件现有背景 Drawable 的副本作为 ripple mask 复用圆角 alpha 轮廓，不使用可能在 selector 背景上误裁内容的 `clipToOutline`；遥控器 OK 键不产生 `MotionEvent`，因此该反馈物理上不会在遥控器触发。释放后清除 ripple foreground，遥控器 OK 的 pressed 态不会触发 ripple。彩色/高亮面（蓝/绿/红/黄/琥珀、modal 主/危险按钮）使用 `@color/ui_press_overlay_dark`（加深），深色中性面（文件行、暗按钮、歌单 tile、modal surface/list row/input、设置项、睡眠预设）使用 `@color/ui_press_overlay_light`（提亮）。遥控器聚焦“展开文字”按钮（浏览页紧凑按钮、播放页底部展开按钮）时，宽度以 Material decelerate（展开约 200ms）/ accelerate（收起约 150ms）非线性动画过渡；图标统一使用 Button compound drawable，收起态清空文字与 drawablePadding，并将半个 `@dimen/ui_space_sm` 从右侧 padding 对称移至左侧，以总 padding 不变的方式补偿空文字内容盒固定左偏；展开时恢复对称 padding 与图文间距。文字强制单行，并在图标和文字都就位后测量展开宽度；展开过程裁切原文而不逐帧重算省略号，避免收起错位、展开目标偏窄、播放页高度跳变和文案跳变。动画由 `ValueAnimator`/Choreographer 跟随设备 VSYNC，不固定 60 FPS；90/120/144 Hz 设备会按实际刷新率取样，相邻帧舍入为同一整数宽度时跳过重复 `requestLayout()`，减少高刷下整行重复测量。每次状态切换会先取消同控件的旧动画，避免快速切焦竞争。触屏模式、未布局或宽度为 0 时直接落定目标宽度不播动画，保证单测同步契约与触屏互斥。动效自动遵守系统“动画时长缩放”。`foreground` 涟漪在 API 23+ 渲染，API 21–22 为 no-op。详细规范见 `DESIGN.md`“动效与微交互”小节。
33. `BaseActivity` 会在 Android 6.0 及以上请求当前物理分辨率所支持的最高刷新率 display mode，不跨分辨率切换；因此高刷实机可让 Choreographer 动画按 90/120/144 Hz VSYNC 取样。最终模式仍受系统省电、温控和外接显示器策略控制。MuMu 当前仅暴露 60 Hz mode，模拟器只能用于 60 FPS 帧耗验证。
34. 文件浏览新增列表 / 宫格切换并全局记住最后选择，NAS 与本地来源共用。视图按钮默认只显示当前模式 SVG，聚焦后展开当前模式文案“列表视图 / 宫格视图”，图标和文字都不表示待切换目标；点击后立即回显新的当前模式。宫格列数会随 80%–120% 全局缩放后的可用宽度自动调整，卡片仅显示封面/类型图标和文件名；目录显示文件夹 SVG，音频优先显示同目录或内嵌封面，无封面时显示音频 SVG，不使用 `default_cover`。宫格封面复用播放页内存与内容寻址磁盘缓存，同内容只保存一份；SMB 内嵌封面使用独立的严格局部标签探测，不会因解析失败而复制完整音频。播放页、收藏页和宫格统一采用尺寸探测后采样解码，缓存 Bitmap 最长边限制为 1024px，避免 5K 等高分辨率封面在 ARGB_8888 解码后膨胀到百兆像素缓冲并触发 Canvas 崩溃。
...
列表 / 宫格切换、宫格列数或缩放后的可用宽度变化会纳入渲染指纹并触发必要重建；宫格快速定位的整页跳转数量按“可见行数 × 当前列数”计算，保持与列表模式一致的整屏跳转语义。
...
若发现新版本，会弹窗询问是否下载；更新提示和稍后时间选择不显示左上角分区标签。“稍后”“下载并安装”及稍后时间选项均支持遥控器确认和触屏点按；更新提示或稍后选择列表可通过 Back 或触摸遮罩直接忽略，不写入稍后策略。选择“本次”只跳过当前进程中的下一次自动提示；选择“7天”会在到期前跳过所有自动更新提示，即使期间发布了更高版本也不提示，到期后重新检测最新版本；选择“下个版本”只跳过当前发现版本，若之后检测到更高版本会重新提示。用户确认下载后前台显示下载进度弹窗，内容包含文件名、实时网速、已下载/总大小和蓝色从左向右填充的进度条，不再显示百分比，也不使用系统流动进度背景。下载任务会从 APK 响应头读取 `Content-Length` 作为总大小；若服务器未返回总大小，则保守显示已下载大小并保持稳定轨道。下载完成后通过系统 APK 安装页面继续安装。APK 临时保存在 `cacheDir/updates/`，并通过 `FileProvider` 只授权该缓存目录给系统安装器读取。
