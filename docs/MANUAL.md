...
3. 当前播放能力已支持临时队列、默认收藏夹和自定义播放列表的本地 SQLite 持久化；设置页已支持该 SQLite 库的本地全量导入/导出与 WebDAV 手动全量上传/下载；尚未实现复杂歌单管理（重命名、排序、批量移动、自动云同步等）与播放历史。
...
22. 设置页新增“备份恢复”分类。当前备份范围限定为显式 SQLite 数据库 `favorites.db`（收藏夹、自定义播放列表与收藏歌曲记录），不包含歌词/封面缓存、播放快照、UI 设置或 SMB 连接配置。本地“导出本地备份”会把当前库全量复制到应用备份目录；“导入本地备份”会先校验备份包含收藏表结构，确认后覆盖当前库。WebDAV 需要先填写服务器地址、用户名、密码和远端目录；“上传到 WebDAV”会先生成本地全量备份，再以固定文件名 `favorites.db` 手动上传；“从 WebDAV 下载并恢复”会下载同名文件到临时位置，校验后覆盖当前库。该功能不做自动同步、后台轮询或启动时自动恢复。
