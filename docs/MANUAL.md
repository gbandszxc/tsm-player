...
34. 文件浏览新增列表 / 宫格切换并全局记住最后选择，NAS 与本地来源共用。宫格列数会随 80%–120% 全局缩放后的可用宽度自动调整，卡片仅显示封面/类型图标和文件名；目录显示文件夹 SVG，音频优先显示同目录或内嵌封面，无封面时显示音频 SVG，不使用 `default_cover`。宫格封面复用播放页内存与内容寻址磁盘缓存，同内容只保存一份；SMB 内嵌封面使用独立的严格局部标签探测，不会因解析失败而复制完整音频。播放页、收藏页和宫格统一采用尺寸探测后采样解码，缓存 Bitmap 最长边限制为 1024px，避免 5K 等高分辨率封面在 ARGB_8888 解码后膨胀到百兆像素缓冲并触发 Canvas 崩溃。
