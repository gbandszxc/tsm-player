...
27. 浏览页右上角在“设置”左侧新增来源模式按钮，默认 NAS 模式，选择会持久化。本地模式使用主共享外部存储目录树，隐藏根级连接管理与 Menu 连接管理入口；排序、焦点锚点、播放、收藏、历史、定位、同名 LRC、内嵌标签和同目录封面继续复用现有链路。本地模式首次进入会引导系统“所有文件访问权限”，未授权时显示“前往授权”。
