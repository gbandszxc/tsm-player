...
3. 当前播放能力已支持临时队列、默认收藏夹和自定义播放列表的本地 SQLite 持久化；尚未实现复杂歌单管理（重命名、排序、批量移动、云同步等）与播放历史。
...
11. SMB 浏览页“播放控制”行已在顺序播放前新增“收藏”入口按钮，点击会进入 `FavoritesActivity`。当前 `FavoritesActivity` 仅为空壳页面，用于保持 Task 5 后的启动与编译链路；完整收藏宫格页留待后续任务实现。浏览页收藏、顺序播放、随机播放按钮未聚焦时折叠为图标，获得焦点时展开显示文案，右侧“回到当前播放/继续上次播放”按钮仍使用 `0dp + weight=1` 自适应剩余宽度。
12. `FavoritesActivity` 已实现收藏页宫格与列表播放：入口页第一个 tile 为“添加播放列表”，之后显示默认收藏夹与自定义播放列表；tile 封面优先使用最后加入歌曲 artwork，缺失时使用 `default_cover`。点击播放列表进入纵向歌曲列表，点击歌曲会从当前收藏列表构建 Media3 队列并跳到所选 index 播放；右侧删除按钮与失效歌曲确认弹窗都只移除收藏记录，不删除 SMB 原文件。该 Activity 已在 Manifest 注册，本次未重复注册。
