...
21. 版本里程碑：`1.0.0`（首版发布）、`1.0.1`（歌词/封面修复）、`1.0.2`（设置页与全局缩放修复）、`1.0.3`（上次播放记忆、歌词封面磁盘缓存、清理缓存设置、全屏歌词缓存修复）、`1.0.4`（SMB 封面/标签多格式快速探测与回退策略优化）、`1.0.5`（TV 浏览页长列表快速定位、目录焦点锚点恢复、快速定位悬浮比例条可见性修复）、`1.0.6`（应用内自动/手动更新检查、播放页图片全屏预览）、`1.0.7`（睡眠模式、播放页面封面图全屏预览、长音频进度条优化、音乐标签信息回显修复）。
...
30. 版本 `1.0.7`（2026-05-24）：新增睡眠模式，可定时退出 app 并在设备管理器授权后请求休眠；新增播放页面封面图全屏预览；优化一小时以上长音频进度条拖动与遥控快进快退卡顿；修复切换状态后音乐标签信息回显失效。
