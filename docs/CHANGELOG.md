...
21. 版本里程碑：`1.0.0`（首版发布）、`1.0.1`（歌词/封面修复）、`1.0.2`（设置页与全局缩放修复）、`1.0.3`（上次播放记忆、歌词封面磁盘缓存、清理缓存设置、全屏歌词缓存修复）、`1.0.4`（SMB 封面/标签多格式快速探测与回退策略优化）、`1.0.5`（TV 浏览页长列表快速定位、目录焦点锚点恢复、快速定位悬浮比例条可见性修复）、`1.0.6`（应用内自动/手动更新检查、播放页图片全屏预览）、`1.0.7`（睡眠模式、播放页面封面图全屏预览、长音频进度条优化、音乐标签信息回显修复）、`1.0.8`（收藏夹与自定义播放列表、文件列表大小/时间与排序、统一项目风格 modal、歌词切歌状态修复）。
...
29. 发版脚本补充（2026-05-10）：新增 `scripts/release.ps1`，可复用执行 Release 构建、校验 `armeabi-v7a`/`arm64-v8a` 两个 APK，并通过 `gh` 创建或更新 GitHub Release 资产。
30. 版本 `1.0.7`（2026-05-24）：新增睡眠模式，可定时退出 app 并在设备管理器授权后请求休眠；新增四种播放模式；优化一小时以上长音频进度条拖动与遥控快进快退卡顿；修复切换状态后音乐标签信息回显失效。
31. 版本 `1.0.8`（2026-05-31）：新增收藏夹与歌曲收藏功能，支持自定义播放列表；文件列表新增文件大小、最后修改时间回显与排序；项目内 Android 原生 modal 统一重绘为项目风格窗体；修复歌曲自然结束切到下一首时，首句歌词出现前旧歌词状态未重置的问题。
