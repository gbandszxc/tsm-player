...
17. 设置能力终态包含：全局缩放档位（`80-120%`、5% 步进）持久化、播放页/全屏歌词字号可配置（`14sp-56sp`）、休眠保护开关（`FLAG_KEEP_SCREEN_ON`）、关于页 GitHub 入口。
...
21. 版本里程碑：`1.0.0`（首版发布）、`1.0.1`（歌词/封面修复）、`1.0.2`（设置页与全局缩放修复）、`1.0.3`（上次播放记忆、歌词封面磁盘缓存、清理缓存设置、全屏歌词缓存修复）、`1.0.4`（SMB 封面/标签多格式快速探测与回退策略优化）、`1.0.5`（TV 浏览页长列表快速定位、目录焦点锚点恢复、快速定位悬浮比例条可见性修复）、`1.0.6`（应用内自动/手动更新检查、播放页图片全屏预览）、`1.0.7`（睡眠模式、播放页面封面图全屏预览、长音频进度条优化、音乐标签信息回显修复）、`1.0.8`（收藏夹与自定义播放列表、文件列表大小/时间与排序、统一项目风格 modal、歌词切歌状态修复）、`1.0.9`（播放历史、备份恢复与 WebDAV 同步、播放页 metadata 回显修复、定位锚点冲突修复）、`1.0.10`（i18n 国际化适配、异形屏全屏显示优化、设置页触屏交互优化）。
...
32. 版本 `1.0.9`（2026-06-06）：新增播放历史功能；新增备份恢复功能，支持数据导入导出与 WebDAV 同步；修复部分场景回到播放页 metadata 不显示或显示不全的问题；修复定位按钮和目录恢复锚点冲突导致焦点目标被覆盖的问题。
33. 版本 `1.0.10`（2026-06-13）：新增 i18n 国际化适配，增加英文语言支持（自动检测/手动切换）；优化异形屏默认全屏显示；优化设置页左侧分类列表触屏滚动交互。
34. 版本 `1.1.0`（2026-07-17）：新增本地音乐播放支持与文件页宫格/列表切换；优化 WAV 格式音频封面加载，以及按钮聚焦展开交互动画。
