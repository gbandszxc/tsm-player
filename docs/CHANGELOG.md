...
21. 版本里程碑：`1.0.0`（首版发布）、`1.0.1`（歌词/封面修复）、`1.0.2`（设置页与全局缩放修复）、`1.0.3`（上次播放记忆、歌词封面磁盘缓存、清理缓存设置、全屏歌词缓存修复）、`1.0.4`（SMB 封面/标签多格式快速探测与回退策略优化）、`1.0.5`（TV 浏览页长列表快速定位、目录焦点锚点恢复、快速定位悬浮比例条可见性修复）、`1.0.6`（应用内自动/手动更新检查、播放页图片全屏预览）。
